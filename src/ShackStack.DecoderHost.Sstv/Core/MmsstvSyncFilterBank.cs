...
internal sealed class MmsstvSyncFilterBank
{
    ...
    {
        if (_lastOffsetHz == toneBank.AfcFrequencyOffsetHz)
        {
            return;
        }
    ...
    public SyncFilterSnapshot ProcessSample(float sample)
    {
        ...
        return new SyncFilterSnapshot(
            Square(_lpf1080.Process(_tone1080.Process(scaled))),
            Square(_lpf1200.Process(_tone1200.Process(scaled))),
            Square(_lpf1320.Process(_tone1320.Process(scaled))),
            Square(_lpf1900.Process(_tone1900.Process(scaled))),
            Square(_lpfFsk.Process(_toneFsk.Process(scaled))));
    }
