...
public sealed class PythonSstvDecoderHost : ISstvDecoderHost, IDisposable
{
    ...
    private SstvDecoderConfiguration _configuration = new("Martin M1", "14.230 MHz USB", 0, 0);
    private bool _isRunning;

    public PythonSstvDecoderHost(IAudioService audioService)
    {
        _audioService = audioService;
        _workerLaunch = BundledDecoderWorkerLocator.Resolve("sstv_sidecar_worker");
        ...
        _telemetry.OnNext(new SstvDecoderTelemetry(
            false,
            _workerLaunch.Exists ? "Python SSTV worker ready to launch" : $"Worker missing: {_workerLaunch.DisplayPath}",
            "Python SSTV sidecar",
            0,
            _configuration.Mode));
            ...
            try
            {
                ...
                if (string.Equals(type, "telemetry", StringComparison.OrdinalIgnoreCase))
                {
                    _telemetry.OnNext(new SstvDecoderTelemetry(
                        ...
                        root.TryGetProperty("activeWorker", out var workerEl) ? workerEl.GetString() ?? "Python SSTV sidecar" : "Python SSTV sidecar",
