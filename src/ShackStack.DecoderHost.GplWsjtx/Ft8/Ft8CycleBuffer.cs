...
internal sealed class Ft8CycleBuffer
{
    private readonly List<float> _pending = new(Ft8Constants.InputSamplesPerCycle * 2);
    ...
    {
        if (sampleRate != Ft8Constants.InputSampleRate || channels <= 0)
        {
        ...
        while (_pending.Count >= Ft8Constants.InputSamplesPerCycle)
        {
            var cycle = _pending.GetRange(0, Ft8Constants.InputSamplesPerCycle).ToArray();
            _pending.RemoveRange(0, Ft8Constants.InputSamplesPerCycle);
            cycles.Add(cycle);
        }
    ...
    {
        var cycleTicks = TimeSpan.FromSeconds(Ft8Constants.CycleSeconds).Ticks;
        ...
        return (int)Math.Round(secondsUntilNext * Ft8Constants.InputSampleRate, MidpointRounding.AwayFromZero);
    }
