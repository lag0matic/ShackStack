# Digital And Weak-Mode Status - 2026-04-27
...
| --- | --- | --- | --- | --- | --- |
| Voice / SSB | Yes | Yes | Functional | Native ShackStack radio/audio path | Voice desk supports PTT, audio routing, POTA staging, and Longwave logging. |
...
| FT4 | Yes | Yes | Functional | WSJT-X-derived GPL sidecar/runtime | Same operating family as FT8. Useful when signals are short bursts and FT8 does not explain what is heard. |
...
| WSPR | Yes | Deferred | Functional RX | WSJT-X `wsprd.exe` | RX monitor works with two-minute cycle behavior. TX is not urgent because it is propagation beaconing, not QSO work. |
...
| SSTV | Yes | Yes | Functional | MMSSTV-shaped native sidecar | Strongest focus is Martin, Scottie, Robot, and PD families. RX/TX, templates, FSKID, `%tocall`, slant/sync, and archive behavior are usable. |
...
| CW | Experimental | Basic keying/TX exists | Rough | Python adaptive worker only | CW remains the dragon. It decodes somewhat, but not reliably enough to call solved; old alternate ggmorse bridge work has been removed to reduce confusion. |
...
| FreeDV live integration | Medium | Medium | HF digital voice is source-backed and now harness-green through RADEV1. Live RF validation, latency, and operator polish are the remaining work. |
| RTTY TX | Medium-Low | Medium | Useful eventually, but not as urgent as RX because the radio and other tools already cover many RTTY needs. |
| FST4 / FST4W | Low-Medium | Medium | HF-capable weak-signal modes. WSPR already covers much of the passive propagation-monitoring need. |
| JT65 / JT9 | Low | Medium-Low | Older HF compatibility modes. Keep possible, not a near-term focus. |
| ARDOP / Winlink-style integration | Low | Medium-Low | HF capable, but the workflow is mailbox/session based rather than normal live decode. Better as a dedicated future desk. |
...
| SSTV | Live RX and TX usable, including FSKID and template overlay fixes. |
